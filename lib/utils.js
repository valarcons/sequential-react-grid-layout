...
export function compact(
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      // Check if there is enough space on the current row
      if (totalWidth + l.w > cols) {
        // Move to the next row
        currentRow += currentRowHeight;
        totalWidth = 0;
        currentRowHeight = 1; // Reset the row height for the new row
      }
      l.y = currentRow;
      l.x = totalWidth;
      totalWidth += l.w;
      currentRowHeight = Math.max(l.h, currentRowHeight); // Update the row height if necessary
    }
...
export function moveElement(
  ...
  if (isUserAction) {
    isUserAction = false;
    if (oldX === x) isLeftShift = oldY - (y ?? 0) <= 0;
    if (oldY === y) isLeftShift = oldX - (x ?? 0) <= 0;
  }
...
export function moveElementAwayFromCollision(
  ...
  const isTileWrapping = isLeftShift ? itemToMove.x - 1 < 0 : itemToMove.x + 1 >= cols;
